...
library(readr)
DN1_mature_count_merge <- read.csv("~/Desktop/result/DGCR8/mirdeep2/DN1_miRNA_count_merge.csv",header=FALSE)
View(DN1_mature_count_merge) 
library(readr)
DN2_miRNA_count_merge <- read.csv("~/Desktop/result/DGCR8/mirdeep2/DN2_miRNA_count_merge.csv",header=FALSE)
View(DN2_miRNA_count_merge)
library(readr)
DT1_miRNA_count_merge <- read.csv("~/Desktop/result/DGCR8/mirdeep2/DT1_miRNA_count_merge.csv",header=FALSE)
View(DT1_miRNA_count_merge)
library(readr)
DT2_miRNA_count_merge <- read.csv("~/Desktop/result/DGCR8/mirdeep2/DT2_miRNA_count_merge.csv",header=FALSE)
View(DT2_miRNA_count_merge)
colnames(DN1_mature_count_merge)<-c("MiRBase_ID","DN1")
colnames(DN2_miRNA_count_merge)<-c("MiRBase_ID","DN2")
colnames(DT1_miRNA_count_merge)<-c("MiRBase_ID","DT1")
colnames(DT2_miRNA_count_merge)<-c("MiRBase_ID","DT2")
A<-sort(DN1_mature_count_merge$CeneID)
head(A)
DN<-merge(DN1_mature_count_merge,DN2_miRNA_count_merge,by="MiRBase_ID")
DT<-merge(DT1_miRNA_count_merge,DT2_miRNA_count_merge,by="MiRBase_ID")
merge<-merge(DN,DT,by="MiRBase_ID")
write.csv(merge,"miRNA_all_count.csv",row.names = F)


library(readr)
miRNA_all_count <- read_csv("~/Desktop/result/DGCR8/mirdeep2/miRNA_all_count.csv")
dataFrame<-miRNA_all_count

DESeq2_result_prefix<-"DGCR8_KO_vs_WT_miRNA"

countMatrix <- as.matrix(dataFrame[2:5])
# countMatrix <- as.matrix(dataFrame[2:5])
rownames(countMatrix) <- dataFrame$MiRBase_ID
condition <- factor(c(rep("WT",2), rep("DGCR8_KO",2)), levels = c("WT","DGCR8_KO"))
...
########################################
miR_Family_Info <- read_csv("~/03_database/07_TargetScanhuman/miR_Family_Info.csv")
...
Plot_prefix<-"DGCR8_KO_vs_WT_miRNA"
...
heatmap_data<-heatmap_data[,c(8,9,10,11)]
