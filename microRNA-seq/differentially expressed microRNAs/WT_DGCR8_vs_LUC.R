...
dataFrame<-merge_mature[,c(1,2,3,7,8,9,10)]

DESeq2_result_prefix<-"WT_DGCR8_vs_LUC"

countMatrix <- as.matrix(dataFrame[2:7])
# countMatrix <- as.matrix(dataFrame[2:5])
rownames(countMatrix) <- dataFrame$miRNA_ID
condition <- factor(c(rep("LUC",3), rep("WT_DGCR8",3)), levels = c("LUC","WT_DGCR8"))
